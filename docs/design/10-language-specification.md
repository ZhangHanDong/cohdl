...
Accepted via RFC-029, see RFC-029: Package dependency versioning — std becomes a versioned registry library + DR-035. Closes a real, three-times-disclosed gap: RFC-016/017's Non-goals both explicitly deferred versioning/publishing mechanics to "once real libraries exist," and DR-024's Consequences named it directly ("no versioning/pinning mechanism for pad references... mirroring the same absence of version pinning at every other use-based resolution point in the language today"). Triggered by Tony's direct observation: hardware's essence — a design once fabricated cannot be hotfixed — demands strict, exact version control, categorically stricter than software's semver-range convention.
...
- [dependencies] is a new manifest section. Every entry is name = "X.Y.Z" — an exact semver triple. No range operators are ever valid grammar (^, ~, >=, <, *, , are all rejected at manifest-parse time) — a permanent language rule, not an interim restriction. [package] version (previously parsed and discarded) becomes real: every package, std included, declares its own version.
- cohdl.lock (TOML, one [[package]] table per dependency, parallel in shape to design.lock, RFC-005) records each resolved dependency's exact version and a content hash — a hash over the dependency's full package content (.cohdl source, #[doc(...)]-referenced documents, footprint symbols). On first resolution the compiler writes this row; on every subsequent build it re-hashes the resolved content and compares against the locked hash — a mismatch is a hard compile error naming the package and both hashes, never a silent proceed.
- Updating a pin is always a deliberate, visible act: cohdl update (or hand-editing [dependencies] to a new exact version, then rebuilding) is the only sanctioned way a locked hash changes. An ordinary cohdl build never rewrites an existing cohdl.lock row.
- std is retired as a find_std_dir-located singleton (the previous hardcoded search: --std flag → COHDL_STD env → path next to the compiler binary → cwd) and becomes an ordinary versioned package under this exact mechanism — every project implicitly depends on some pinned std version (no discovery step needed, mirroring Rust's std/no_std precedent). --std/COHDL_STD survive only as local development overrides, and using either triggers a mandatory, unsuppressable warning that the build is not using the locked std and its result must not be treated as reproducible.

Rules:

- Manifest/lock verification happens before any .cohdl file is opened — a new pipeline stage gating everything RFC-001–028 already check, not a parameter any existing checking/emission pass receives.
- A [dependencies] entry with no corresponding cohdl.lock row triggers first-resolution (a new row is written) — this is the only case an ordinary build may add to cohdl.lock.
- cohdl.lock is committed to the consuming project's own version control, the same convention design.lock already establishes — it is the durable, diffable record of exactly which library content a given design was built against, answerable months or years later without any external registry lookup.
- Error codes reserve a new block (package resolution — a new kind of mistake per RFC-011's organizing principle, since no existing block owns "which dependency version resolved"): invalid-range-syntax in [dependencies], hash mismatch against cohdl.lock, unresolvable named exact version.
- Not a build cache or cross-compiler-version byte-identity guarantee — this mechanism guarantees which library content a build resolves against, not that two different compiler versions produce identical output from that content (RFC-005's designator determinism and cohdl fmt's idempotence remain the relevant guarantees for that).
- Not registry hosting/publishing infrastructure (a package index, cohdl publish, yanking policy) — this RFC defines the manifest/lock/resolution mechanism assuming packages are available on disk, mirroring RFC-016/017's own scope boundary; real hosting remains separate, not-yet-proposed future work.

# Package registry (registry.cohdl.org)

Accepted via RFC-030, see RFC-030: registry.cohdl.org — the package registry service + DR-036. Closes the hosting layer RFC-029's own Non-goals explicitly deferred ("registry hosting/publishing infrastructure... real hosting remains separate, not-yet-proposed future work"). Composes directly with RFC-029's already-Accepted exact-version/cohdl.lock/hash-verification mechanism — this section adds no new version-resolution rule; it specifies where the content those mechanisms resolve against actually comes from.
...
Three closed namespace tiers, each legible in the name itself with no side-channel lookup needed:

- Tier 1 — bare name (std, matching RFC-016's existing package-name grammar unchanged). Reserved exclusively for packages published by the CoHDL project's own official account — never first-come-first-served. A publish attempt to an unclaimed bare name is rejected.
- Tier 2 — @brand/name. brand must be a human-verified manufacturer account — verification is a real, gated registry process, not self-service. Publishing under an unclaimed or unverified brand prefix is rejected, naming the verification requirement.
- Tier 3 — @contrib/name. Any authenticated account may publish any not-yet-taken @contrib/name — first-come-first-served within this one shared prefix only, never colliding with or confusable for tier 1/2 trust.

A package's tier is determined structurally by its name's own shape, never a separate metadata flag — @sparkfun/power and @contrib/power are two entirely distinct, non-colliding names.
...
- No per-reference versioning/pinning for individual pad references within a package — a footprint always resolves to whatever pad symbol is in its own package's currently-resolved content. Since RFC-029, that package-level content itself is version-pinned and hash-verified via cohdl.lock — so a pad reference is no longer an unpinned moving target across builds; it moves only when an author deliberately runs cohdl update on that package.
...
Accepted via RFC-031, see RFC-031: Silkscreen graphics for footprints + DR-037. Closes a real gap in the pad/footprint model above: silkscreen_ref is a fixed-purpose reference-designator text placement (positions KiCad's own fp_text reference "REF**") and courtyard draws exactly one fixed keep-out rectangle — neither can draw any other silkscreen graphic. There was previously no way to express a pin-1 marker or a diode/capacitor polarity indicator at all.
...
silkscreen { ... } is a new, optional footprint-body block — a fourth sibling alongside pad, mount_hole, courtyard, silkscreen_ref (unchanged, untouched by this RFC). Its content is either raw drawable primitives or semantic marker sugar that expands to them.

Four closed graphic primitive kinds (all Length-typed per RFC-001; FILL closed to {none, solid}):

- line from (x1, y1) to (x2, y2) width W
- circle at (x, y) radius R width W [fill FILL] — FILL defaults none.
- arc at (x, y) radius R start_angle A1 end_angle A2 width W — angles are any integer degree 0-360, not restricted to the cardinal {0,90,180,270} set used elsewhere (arcs need a genuinely continuous angle, unlike whole-component/pad orientation).
- polygon [(x1, y1), (x2, y2), (x3, y3), ...] [fill FILL] — 3 or more vertices; FILL defaults solid.

These reuse a new, purpose-built SilkShape/SilkGraphic/SilkFill type family — deliberately not RFC-018's PadShape (whose fields — width/height/drill/roundrect_ratio — are tied to copper/soldermask/drill geometry with no meaning for a silkscreen stroke).

Two semantic marker shorthands — sugar that expands to real, checked primitives at compile time:
...
- Zero DRC/schematic-correctness impact, by construction — identical to every other footprint-body construct; silkscreen { ... }'s content is never read by the type checker, residual DRC, designator allocator, or pin-obligation checks.
